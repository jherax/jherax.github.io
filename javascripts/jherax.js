 ...
 *  http://jquery.com/
 -------------------------------------
 *  Abstract:
 *  This is a library of utilities for JavaScript and jQuery, which includes tools for data validation, text formatting,
 *  tooltips, positioning elements, JSON manipulation, cloning objects, sorting arrays, resource injection, among others.
 -------------------------------------
...
(function (window, $, jherax) {
    ...
    function handlerExist (dom, eventName, namespace) {
        var i,
            noop = {},
            listener,
            listeners = ($._data(dom, 'events') || noop)[eventName] || [],
            totalListeners = listeners.length;
        for (i = 0; i < totalListeners; i += 1) {
            listener = listeners[i];
            if (namespace === (listener.namespace || (listener.data || noop).handler)) {
                return true;
            }
        }
        return false;
    ...
    function getQueryToObject (url, key) {
        ...
        if (!url) url = window.location.search;
        $.each(url.split(/[?&#]/g), function(i, param) {
            m = param.match(pair);
    ...
    var formatDate = (function () {
        ...
        return function (options) {
            ...
            var opt = Object.assign({}, defaults, options),
                ...
                sourceParts = opt.inputFormat.split(/\W/);
            for (i in sourceParts) {
                if (_d.test(sourceParts[i])) { d = dateParts[i]; continue }
                if (_m.test(sourceParts[i])) { m = dateParts[i]; continue }
                if (_y.test(sourceParts[i])) { y = dateParts[i]; continue }
            }
    ...
    var dateToString = (function () {
        var _TYPE = (/\[object\s(?:String|Number|Date)\]/),
            _DATE = (/[dMy]+/g),
            _HOUR = (/[Hhms]+/g);
        function _fillZero (n) { return ("0" + n.toString()).slice(-2); }
        function fnDate (o) {
            return (o.ISO8601 ? "yyyy-MM-dd" : _language.dateFormat).replace(_DATE, function (match) {
                switch (match) {
                    case "dd": return _fillZero(o.date.getDate());
                    case "MM": return _fillZero(o.date.getMonth() + 1);
        ...
        function fnTime (o) {
            return (o.ISO8601 ? "HH:mm" : _language.timeFormat).replace(_HOUR, function (match) {
                ...
                switch (match) {
                    case "HH": return _fillZero(o.date.getHours());
                    case "hh": return _fillZero(h === 12 ? 12 : h % 12);
                    case "mm": return _fillZero(o.date.getMinutes());
                    case "ss": return _fillZero(o.date.getSeconds());
                }
            });
        }
    ...
    var isValidFormat = (function () {
        function _formatter (format) {
            return ("^" +
            escapeRegExp(format).replace(/[dMyHhms]+/g, function (match) {
                switch (match) {
        ...
        var validator = {
            date: function (text, pattern) {
                //Validates the date format according to regional setting
                pattern = pattern || new RegExp(_formatter(_language.dateFormat));
                return _validate(text, pattern);
            },
            time: function (text, pattern) {
                //Validates the time format: HH:mm:ss
                pattern = pattern || (/^([0-1][0-9]|[2][0-3]):([0-5][0-9])(?::([0-5][0-9])){0,1}$/);
                return _validate(text, pattern);
            },
            datetime: function (text, pattern) {
                //Validates the date-time format according to regional setting
                pattern = pattern || new RegExp(_formatter(_language.dateFormat + " " + _language.timeFormat));
                return _validate(text, pattern);
            },
            email: function (text, pattern) {
                //Validates an email address
                pattern = pattern || (/^([0-9a-zñÑ](?:[\-.\w]*[0-9a-zñÑ])*@(?:[0-9a-zñÑ][\-\wñÑ]*[0-9a-zñÑ]\.)+[a-z]{2,9})$/i);
                return _validate(text, pattern);
            },
            url: function (text, pattern) {
                //Validates an url address
                pattern = pattern || (/((?:http|ftp|https):\/\/[\w\-_]+(?:\.[\w\-_]+)+(?:[\w\-\.,@?\^=%&:\/~\+#]*[\w\-\@?\^=%&\/~\+#])?)/gi);
                return _validate(text, pattern);
            },
            ipv4: function (text, pattern) {
                //Validates an IP address v4
                pattern = pattern || (/^(?:(?:25[0-5]|2[0-4]\d|[01]\d\d|\d{1,2})\.){3}(?:25[0-5]|2[0-4]\d|[01]\d\d|\d{1,2})$/);
                return _validate(text, pattern);
            },
            password: function (text, pattern) {
                //Validates the password strength (must have 8+ characters, 1+ number, 1+ uppercase)
                pattern = pattern || (/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/);
                return _validate(text, pattern);
            },
            latitude: function (text, pattern) {
                //Validates latitudes range from -90 to 90
                pattern = pattern || (/^-?([1-8]?[1-9]|[1-9]0|0),{1}\d{1,6}$/);
                return _validate(text, pattern);
            },
            longitude: function (text, pattern) {
                //Validates longitudes range from -180 to 180
                pattern = pattern || (/^-?([1]?[1-7][1-9]|[1]?[1-8][0]|[1-9]?[0-9]),{1}\d{1,6}$/);
                return _validate(text, pattern);
            }
        };
    ...
    var isValidDate = (function () {
        function _parser (date, e) {
            var type, dt, part, length, i, y, m, d;
            ...
            for (i = 0, length = part.length; i < length; i += 1) {
                if (y.test(part[i])) { date = date.replace("y", dt[i]); continue; }
                if (m.test(part[i])) { date = date.replace("M", dt[i]); continue; }
                if (d.test(part[i])) { date = date.replace("d", dt[i]); continue; }
            }
            dt.splice(0, 3);
            return new Date(date + " " + dt.join(":"));
        }
        return function (obj, o) {
            ...
            if ( e.error) { return isInput && showTooltip(obj, _language.dateFormatError); }
            if ( o.isFuture && dif < 0) { return isInput && showTooltip(obj, o.warning || _language.dateIsLesser, o.position); }
            if (!o.isFuture && dif > 0) { return isInput && showTooltip(obj, o.warning || _language.dateIsGreater, o.position); }
            return true;
        };
    }());
    //-----------------------------------
    ...
    (function () {
        ...
        $.fn.easyValidate = function (o) {
            ...
            return this.each(function (index, btn) {
                ...
                if (isFunction(d.onBeforeTooltip)) {
                    evt = nsEvents("beforeTooltip", "jsu-easyValidate-" + index);
                    ...
                    $(".vld-required," + _ALLFILTERS).each(function (i, input) {
                        ...
                        if ($input.hasClass("vld-required") && ((tag === "select" && (_fnValidateFirstItem(input, d) || input.value === "0")) ||
                            ...
                            if (tag === "span" || inputType.isCheck(input)) {
                                if (tag === "input") { checkbox = $input; }
                                else { checkbox = $input.find("input:first-child"); }
